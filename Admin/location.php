...
<form method="POST" action="location.php"> 
   ...
   <p><input type="submit" value="Search" name="locationCapacitySearch"></p>
</form>
...
<p><font size="3">To delete a certain row of data please type the Location ID here:</p>
    <form method="POST" action="location.php"> 
   <p><input type="text" placeholder="type Location ID here.." name="deleteLocationIDRow" size="18">
...
if ($db_conn) {
    ...
    if (array_key_exists('deleteCondition', $_POST)) {
           $DeleteCertainRow = $_POST['deleteLocationIDRow'];
            executePlainSQL("delete from location where LocationID = " . $DeleteCertainRow . " ");
            OCICommit($db_conn);
        ...
        if (array_key_exists('insertsubmit', $_POST)) {
            ...
            if (array_key_exists('deleteAll', $_POST)) {
                ...
                if (array_key_exists('updateValueAction', $_POST) || array_key_exists('updateValue', $_POST)) {
                    ...
                    executeBoundSQL("update location set " . $_POST['updateValue'] . "=:bind1 where locationID=:bind3 ", $alltuples);
                    OCICommit($db_conn);
                }
            }
        } 
    }
    $lol = array_key_exists('locationSearch', $_POST) || array_key_exists('locationCapacitySearch', $_POST) || array_key_exists('updateValue', $_POST);
    $lol = !$lol;
    if ($_POST && $success && $lol) {
        ...
        if (array_key_exists('locationSearch', $_POST)) {
            ...
            $result = executePlainSQL("select * from location where locationID like '%" . $eventsearched . "%'");
        } elseif (array_key_exists('locationCapacitySearch', $_POST)) {
            $CapacitySearched = $_POST['updateValueCapacity'];
            $result = executePlainSQL("select * from location where capacity >= " . $CapacitySearched . "");
        } else {
